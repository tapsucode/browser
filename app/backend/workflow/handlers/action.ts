...
export const actionHandler: NodeHandler = async (context: ExecutionContext, params: Record<string, any>, nodeId: string): Promise<ExecutionContext> => {
  ...
  switch (nodeId) {
    ...
    case 'resourceStatus': {
      if (page && page.evaluate) {
        context.resourceStatus = await page.evaluate((type: string) => {
          return Array.from(window.performance.getEntries()).filter((r: any) => r.initiatorType === type).length;
        }, params.resourceType);
      }
      break;
    }
    ...
    case 'click':
      if (page && page.click) {
        await page.click(
          params.selectorValue || variables[params.selectorVariableRef],
    ...
    case 'rightClick':
      if (page && page.click) {
        await page.click(
          params.selectorValue || variables[params.selectorVariableRef],
          { button: 'right', timeout: params.timeout }
        );
      }
      break;
    ...
    case 'type':
      if (page && page.type) {
        await page.type(
          params.selectorValue || variables[params.selectorVariableRef],
          params.text || variables[params.variableRef],
          { delay: params.delay }
        );
      }
      break;
    ...
    case 'selectOption':
      if (page && page.selectOption) {
        await page.selectOption(
          params.selectorValue || variables[params.selectorVariableRef],
          params.value || variables[params.variableRef]
        );
      }
      break;
    ...
    case 'upload':
      if (page && page.setInputFiles) {
        await page.setInputFiles(
          params.selectorValue || variables[params.selectorVariableRef],
          params.filePath || variables[params.filePathVariableRef]
        );
      }
      break;
    ...
    case 'download':
      if (page && page.goto) {
        await page.goto(
          params.url || variables[params.urlVariableRef]
        );
      }
      break;
    ...
    case 'scroll':
      if (page && page.evaluate) {
        await page.evaluate(
          ({ dir, amt }: { dir: string; amt: number }) => window.scrollBy(
            0, dir === 'down' ? amt : -amt
          ),
          { dir: params.direction, amt: params.amount }
        );
      }
      break;
    ...
    case 'pressKey':
      if (page && page.keyboard && page.keyboard.press) {
        await page.keyboard.press(
          params.key || variables[params.keyVariableRef]
        );
      }
      break;
    ...
    case 'dragAndDrop':
      if (page && page.dragAndDrop) {
        await page.dragAndDrop(
          params.sourceSelectorValue || variables[params.sourceSelectorVariableRef],
          params.targetSelectorValue || variables[params.targetSelectorVariableRef]
        );
      }
      break;
    ...
    case 'getAttribute':
      if (page && page.getAttribute) {
        variables[params.resultVar] = await page.getAttribute(
          params.selectorValue || variables[params.selectorVariableRef],
          params.attribute
        );
      }
